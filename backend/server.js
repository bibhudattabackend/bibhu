...
const attendanceSchema = new mongoose.Schema({
    ...
    timestamp: { type: Date, default: Date.now }, // Initial attendance timestamp
    firstName: { type: String, required: true },
    lastName: { type: String, required: true },
    emailOrMobile: { type: String, required: true },
    lastMarkedDate: { type: Date, default: Date.now }, // Track last attendance date
    ...
    ] // Array to store subsequent attendance records
});

attendanceSchema.index({ user: 1, 'additionalAttendance.period': 1 }, { unique: true });
...
app.get('/api/records1', async (req, res) => {
    try {
        // Fetch attendance records and select specific fields
        const records = await Attendance.find()
            .select('firstName lastName lastMarkedDate emailOrMobile'); // Specify the fields you want
...
app.post('/api/attendance', authenticateToken, async (req, res) => {
    try {
        ...
        if (attendance) {
            ...
            const lastAttendance = attendance.additionalAttendance.find(entry =>
                entry.period >= startOfToday && entry.period <= endOfToday
            );

            if (lastAttendance && lastAttendance.isAttendance) {
                return res.status(200).json({ message: 'Attendance already marked for today' });
            } else {
                // Add a new attendance entry for today
                attendance.additionalAttendance.push({
                    timestamp: Date.now(),
                    period: new Date(), // Set period to the current date
                    isAttendance: true
                });
                attendance.lastMarkedDate = new Date(); // Update lastMarkedDate

                await attendance.save();
                return res.status(201).json({ message: 'New attendance recorded for today' });
            }
        } else {
            ...
            await attendance.save();
            return res.status(201).json({ message: 'Attendance marked for the first time' });
        }
    } catch (error) {
...
app.get('/api/courses/:instructorId', async (req, res) => {
 ...
 app.delete('/api/records/:userId', async(req,res)=>{
